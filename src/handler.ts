...
export async function getDomainDetails(domain: string) {
    let isWebServer = await ha.checkForAnyWebServer('https', domain).then((token) => {
        return token;
    });
    let isOnlineWebServer = await ha.checkForOnlineWebServer('https', domain).then((token) => {
        return token;
    });
    let FQDN_Analysis = await fqdn.checkFQDM(domain).then((token) => {
        return token;
    });
    let status = getStatus(FQDN_Analysis);

    let result =
        ha.matchDomain(domain) == -1
            ? 'Inconclusive'
            : {
